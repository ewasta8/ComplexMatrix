...
begin
  ...
  begin
    LblA.Caption := 'Podaj wartość a[i,j]';
  ...
  begin
    LblA.Caption := 'Podaj wartość końców przedziału a[i,j].a a[i,j].b';
...
begin
  ...
  begin
    ShowMessage('Wpisz liczbę całkowitą większą od zera w polu "' + Edit.Name + '".');
    Edit.SetFocus;
  end;
end;
...
begin
  ...
  if TryStrToFloat(TextVal, Value, fs) then
    ...
    ShowMessage('Błędny format liczby zmiennoprzecinkowej w polu "' + Edit.Name + '".');
    Edit.SetFocus;
  end;
end;
...
begin
  ...
  begin
    ShowMessage('Liczba niewiadomych musi być większa od zera');
    Exit;
  end;
  i := 0;
  j := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        LblA.Caption := 'Macierz została wypełniona';
  ...
  try
    ...
    begin
      ...
      if st = 0 then
        ...
        resultText := 'Układ jest osobliwy (st=' + IntToStr(st) + ')';
    end
    else
    begin
      SetLength(xi, n);
    ...
    on E: Exception do
      ShowMessage('Błąd podczas rozwiązywania: ' + E.Message);
  end;
end;
